...
const BS_CFG = {
  ...
  // Use <CONTACT_NAME> and <VENDOR_NAME> as placeholders
  CANNED_RESPONSES: {
    REFERRAL_PROGRAM: `Hi <CONTACT_NAME>,

Great speaking with you this week, I really appreciate you taking the time to walk me through <VENDOR_NAME>'s model and explore how we can work together. I'm excited about the potential for a strong referral partnership between our teams.

As discussed, here's a simple referral fee structure to formalize things on our end:

Referral Fee Structure Proposal

To formalize our partnership, Profitise proposes the following referral fee structure for any clients introduced by <VENDOR_NAME>:

Referral Fee: 4% of the gross lead spend generated by any referred partner.

Duration: Paid monthly in perpetuity.

Scope: This applies to any introductions made by <VENDOR_NAME> that result in a new client purchasing digital leads from Profitise.

Payment Terms: Referral fees will be calculated at the end of each month after any applicable buyer returns have been processed, to ensure accuracy. Reporting and tracking will be managed through Phonexa, our sister company's lead distribution and reporting platform. A shared tracker (Google Sheet) will also be maintained to log referrals, lead spend, and payment history for full transparency.  The payments will be sent Net 30.

Of course, if there are any adjustments or alternative terms you'd like to explore, we're open to discussing and finding a structure that works well for both sides.

Looking forward to your thoughts, have a great weekend!
...
function generateCannedResponse_(templateKey, templateName) {
  ...
  try {
    ...
    // Get template
    const template = BS_CFG.CANNED_RESPONSES[templateKey];
    if (!template) {
      ui.alert('Error', `Template "${templateKey}" not found.`, ui.ButtonSet.OK);
      return;
    }
