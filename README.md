...
> Before there was a CLI, there was a spreadsheet.
>
> Battle Station was the first version of what eventually became A(I)DEN. An AI-assisted vendor review system built entirely inside Google Apps Script and Google Sheets. It connected Gmail, monday.com, Airtable, and the Claude API to surface vendor context, draft email responses, and flag stale follow-ups, all rendered directly inside a Sheets sidebar.
>
> The constraint was the platform: Google Apps Script runs server-side in V8, has a 6-minute execution limit, no persistent process, and no npm ecosystem. Working inside those constraints forced specific architectural patterns: batched API calls, script property caches, and a stateless render model that pre-fetched all vendor data before opening the UI.
>
> This repo is published as a historical snapshot. The credential config block uses `PropertiesService.getScriptProperties()` to load tokens. Real credentials live in GAS Script Properties (Project Settings > Script Properties) and are never checked in. Board IDs and Airtable table references use generic placeholder names. Substitute your own from your monday.com and Airtable URLs.
>
> The system evolved into a Claude Code CLI pipeline ([claude-agentic-ops](https://github.com/andy-bttryzn/claude-agentic-ops)) when the complexity outgrew what a Sheets sidebar could hold. The core patterns (vendor data aggregation, Gmail context assembly, AI-drafted responses, label-driven triage) carried forward directly.
...
This is the main configuration object. Store all credentials in GAS Script Properties (Project Settings > Script Properties), never hardcode them. The config block reads them at runtime:
...
const BS_CFG = {
  ...
  // API Keys - loaded from Script Properties, never hardcoded
  ...
  // Airtable - find IDs in your Airtable URLs
...
To set Script Properties in GAS: open your Apps Script project, go to Project Settings (gear icon), scroll to Script Properties, and add key/value pairs for `MONDAY_API_TOKEN`, `CLAUDE_API_KEY`, and `AIRTABLE_API_TOKEN`.
...
5. **Update configuration**:
   - Update monday.com board/column IDs in `BS_CFG`
   - Update Airtable base and table IDs in `BS_CFG`
   - Update sheet names to match your data

6. **Set up Gmail labels**:
   - Create the label structure above
   - Set up Gmail filters to auto-apply `00.received` and `zzzvendors-{slug}` labels

7. **Run Setup**: Refresh the sheet, then use menu: `A(I)DEN -> Setup A(I)DEN`

8. **Build your list**: `A(I)DEN -> Build List`

9. **Start reviewing**: Use Navigation menu to go through vendors
...
- On next refresh, labels swap from `02.waiting/me` to `02.waiting/customer`
...
This system grew into a Claude Code CLI pipeline as the operational complexity outgrew what a Sheets sidebar could handle. The architecture shifted from stateless GAS functions to persistent Node.js processes with real file I/O, parallel API calls, and a full vendor brief renderer.

See the successor project: [claude-agentic-ops](https://github.com/andy-bttryzn/claude-agentic-ops)
